  ...
  static async run(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!amount || !fromCurrency || !toCurrency) {
        throw new HttpError(400,'Campos fisicos requeridos: amount, fromCurrency, toCurrency')
      }
      if (Array.isArray(amount)) throw new HttpError(400, 'El valor de amount es incorrecto.')
      const parsedAmount = parseFloat(amount)
      if (isNaN(parsedAmount) || parsedAmount <= 0) throw new HttpError(400, 'Cantidad invalidad: Debe ser un numero positivo.')
